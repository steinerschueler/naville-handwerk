#!/usr/bin/env python3
# Baut die 6 Emblem-PNGs (Logik 1:1 aus _gen_final.py), Ausgabe-Ordner als Argument.
# Laedt Assets + Schild aus tmp/_render/ (via _gen.py). Schild dort = tmp/_render/sign.png.
...
def on_creme(assets, drawcol, shadowcol):
    return g.emblem(*assets, drawcol, CREME, shadow=True, shadowcol=shadowcol,
                    wheeldisc=True, disccol=CREME, **GEO).convert("RGB")
def transparent(assets, drawcol):
    return g.emblem(*assets, drawcol, "trans", shadow=False, **GEO)
...
def save_trim(im, path):
    ...
    im.crop((xs.min(),ys.min(),xs.max()+1,ys.max()+1)).save(path)
