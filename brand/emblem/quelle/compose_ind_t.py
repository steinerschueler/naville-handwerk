...
bike=Image.open("bike_ind.png").convert("RGBA").resize((680,340))
bed=Image.open("trailer_bed_t.png").convert("RGBA").transpose(Image.FLIP_LEFT_RIGHT)
wheel=Image.open("wheel_ind_t.png").convert("RGBA")
sign=Image.open(OUT+"/sign.png").convert("RGBA")
W,H=1500,700; c=Image.new("RGBA",(W,H),(0,0,0,0))
...
def contact(cx,w,al): sd.ellipse((cx-w,G-10,cx+w,G+16),fill=ANTH+(al,))
...
for foot in (stand_x-24,stand_x+30):
    d.line([piv,(foot,G-2)],fill=ANTH,width=13)
    d.line([(foot-13,G-2),(foot+13,G-2)],fill=ANTH,width=8)
...
d.line([axle, Q], fill=ANTH, width=12)               # Hauptrohr zur Velo-Achse
d.line([Q, P1], fill=ANTH, width=11); d.line([Q, P2], fill=ANTH, width=11)  # A-Triangulierung
for P in (P1,P2): d.line([(P[0]-12,P[1]),(P[0]+12,P[1])], fill=ANTH, width=10)  # Anlenk-Bracket
d.ellipse([Q[0]-7,Q[1]-7,Q[0]+7,Q[1]+7], fill=ANTH)  # Gelenk
d.ellipse([axle[0]-11,axle[1]-11,axle[0]+11,axle[1]+11], fill=ANTH)  # Kupplung an der Nabe
# Schild fuellt Bett
...
c.save(OUT+"/emblem_ind_trans.png")
